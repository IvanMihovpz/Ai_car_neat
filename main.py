...
def eval_genomes(genomes, config):
    global cars, ge, nets, generation, cars_alive
    ...
    while run:
        ...
        text = font.render(f"Generation: {generation} | Cars Left: {cars_alive}", True, (255, 255, 255))
...
def extract_fitness_from_output(output):
    pattern = r"Population's average fitness: ([\d.]+)"
    matches = re.findall(pattern, output)
    return [float(match) for match in matches]

def plot_fitness_history(fitness_history):
    plt.figure(figsize=(10, 5))
    plt.plot(fitness_history, label='Average Fitness', color='blue')
...
def run(config_path):
    ...
    fitness_history = extract_fitness_from_output(stdout_capturer.content)
